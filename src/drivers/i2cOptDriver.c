...
{
    SysCtlDelay((g_ui32SysClock / 3 / 1000000) * 300); // ~5 µs
    // Wait for the I2C ISR to give the semaphore
    if (xSemaphoreTake(xIC2MasterSemaphore, pdMS_TO_TICKS(500)) != pdTRUE)
    {
    ...
    uint32_t error = I2CMasterErr(I2C2_BASE);
 ...
 * Puts ui8Reg followed by one data byte in *data and transfers
 * over i2c
 */
...
/*
 * Sets slave address to ui8Addr
 * Puts ui8Reg followed by len data bytes in *data and transfers
 * over i2c
 */
bool writeI2Cmul(uint8_t ui8Addr, uint8_t ui8Reg, uint8_t *data, uint8_t len)
{
    if (len == 0)
        return false;
    if (len == 1)
        return writeI2C(ui8Addr, ui8Reg, data);

    I2CMasterSlaveAddrSet(I2C2_BASE, ui8Addr, false); // Write

    // Start burst with register
    I2CMasterDataPut(I2C2_BASE, ui8Reg);
    I2CMasterControl(I2C2_BASE, I2C_MASTER_CMD_BURST_SEND_START);
    if (!waitForAck())
        return false;
 ...
 * Reads one bytee from i2c slave. 
...
bool readI2Cmul(uint8_t ui8Addr, uint8_t ui8Reg, uint8_t *data, uint16_t len)
{
    if (len < 1)
        return false;
    if (len == 1)
        return readI2C(ui8Addr, ui8Reg, data);
